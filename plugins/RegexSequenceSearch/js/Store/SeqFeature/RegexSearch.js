define([
    ...
    return declare(SeqFeatureStore, {
            ...
            while ((match = expr.exec(sequence)) !== null && match.length) {
                ...
                var newStart =
                    strand > 0
                        ...
                        : end -
                          frameOffset -
                          multiplier * (match.index + result.length);
                var newEnd =
                    strand > 0
                        ? start +
                          frameOffset +
        ...
        translateSequence: function (sequence, frameOffset) {
            ...
            slicedSeq = slicedSeq.slice(
                0,
                Math.floor(slicedSeq.length / 3) * 3,
            );

            var translated = "";
            var codontable = new CodonTable();
            var codons = codontable.generateCodonTable(
