    ...
    return declare(null, {
        ...
        _estimateGlobalStats: function (refseq) {
            ...
            var statsFromInterval = function (length, callback) {
                ...
                var end = Math.min(
                    Math.round(sampleCenter + length / 2),
                    refseq.end,
                );
                var features = [];
                ...
                this._getFeatures(
                    ...
                    function (error) {
                        features = array.filter(features, function (f) {
                            return (
                                f.get("start") >= start && f.get("end") <= end
                            );
                        });
                        const correctionFactor =
                            (thisB.getConf("topLevelFeaturesPercent") || 100) /
                            100;
                        callback.call(thisB, length, {
                            featureDensity:
                                (features.length / length) * correctionFactor,
            ...
            var maybeRecordStats = function (interval, stats, error) {
                if (error) {
                    if (
                        error.isInstanceOf &&
                        error.isInstanceOf(Errors.DataOverflow)
                    ) {
                        console.log(
