    ...
    const defaultAutoSql = {
        ...
        fields: [
            ...
            },
            {
                ...
                name: "chromEnd",
                description:
                    "End position plus one (chromEnd – chromStart = size).",
            },
            {
                type: "string",
                name: "name",
         ...
         */
        {
            ...
            cirFobRecur: function (offset, level) {
                ...
                for (var i = 0; i < offset.length; ++i) {
                    var blockSpan = new Range(
                        offset[i],
                        offset[i] + maxCirBlockSpan,
                    );
            ...
            groupBlocks: function (blocks) {
                ...
                for (var i = 0; i < blocks.length; i++) {
                    if (lastBlock && blocks[i].offset - lastBlockEnd <= 2000) {
                        lastBlock.size +=
                            blocks[i].size - lastBlockEnd + blocks[i].offset;
            ...
            createFeature: function (fmin, fmax, opts) {
                ...
                var f = new SimpleFeature({
                    ...
                    id: id
                        ? id
                        : data.start + "_" + data.end + "_" + data.score,
                });
                    ...
                    if (bedColumns[i - 3] !== "." && bedColumns[i - 3] !== "") {
                        ...
                        if (!autoField._requestWorkerCache) {
                            ...
                            autoField._requestWorkerCache = {
                                isNumeric: numericTypes.includes(
                                    autoField.type,
                                ),
                                isArray: !!match,
                                arrayIsNumeric:
                                    match && numericTypes.includes(match[1]),
                            };
                        }
