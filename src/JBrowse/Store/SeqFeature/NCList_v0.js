            ...
            _handleTrackInfo: function (refData, trackInfo, url) {
                if (trackInfo) {
                    ...
                    dojo.forEach(trackInfo.histogramMeta, function (m) {
                        m.arrayParams.urlTemplate =
                            m.arrayParams.urlTemplate.replace(
                                /\{chunk\}/,
                                "{Chunk}",
                            );
                    });
                        ...
                        dojo.forEach(
                            ...
                            function (varname) {
                                ...
                                if (varname in trackInfo)
                                    track.config.style[dest_varname] =
                                        trackInfo[varname];
                            },
                            this,
                        );
                    ...
                    if (trackInfo.subfeatureHeaders) {
                        for (
                            i = 0;
                            i < trackInfo.subfeatureHeaders.length;
                            i++
                        ) {
                            refData.subFieldOrder.push(
                                trackInfo.subfeatureHeaders[i],
                            );
                            refData.subFields[trackInfo.subfeatureHeaders[i]] =
                                i;
                        }
                    }
                    ...
                    refData.stats = {
                        ...
                        featureDensity:
                            trackInfo.featureCount / this.refSeq.length,
                    };
