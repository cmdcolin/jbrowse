        ...
        ],
        {
            ...
            _getFeatures: function (
                ...
                if (this.regionLoaded) {
                    var spans = array.filter(
                        this.regionLoaded.spans,
                        function (span) {
                            return (
                ...
                var fetchAllFeatures = thisB.stores.map(function (store) {
                    ...
                    if (!featureArrays[store.name]) {
                        ...
                        store.getFeatures(
                            ...
                            function () {
                                d.reject(
                                    "Error fetching features for store " +
                                        store.name,
                                );
                            },
                        );
            ...
            evalTree: function (featureArrays, tree, query) {
                if (!tree) {
                    ...
                    return this.toSpan(
                        featureArrays[tree.right().get().name],
                        query,
                    );
                } else if (!tree.hasRight()) {
                    return this.toSpan(
                        featureArrays[tree.left().get().name],
                        query,
                    );
                }
                return this.opSpan(
