    ...
    return declare(SeqFeatureStore, {
        constructor: function (args) {
            ...
            if (
                ...
                this.getRegionFeatureDensities =
                    this._getRegionFeatureDensities;
            }
        },
        ...
        _getCachedFeatureRegions: function (query) {
            ...
            function tilingIsComplete(regions, start, end) {
                ...
                for (i = 0; !tilingComplete && i < regions.length; i++) {
                    if (
                        coverEnd === undefined ||
                        (regions[i].start <= coverEnd &&
                            regions[i].end > coverEnd)
                    ) {
                    ...
                    fillCallback: function (request, callback) {
                        ...
                        get.then(
                            function (data) {
                                var nocacheResponse =
                                    /no-cache/.test(
                                        get.response.getHeader("Cache-Control"),
                                    ) ||
                                    /no-cache/.test(
                                        get.response.getHeader("Pragma"),
                                    );
                                callback(
