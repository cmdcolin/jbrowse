         ...
         */
        {
            ...
            constructor: function (args) {
                ...
                if (args.cram) dataBlob = new BlobFilehandleWrapper(args.cram);
                    ...
                    throw new Error(
                        "must provide either `cram` or `urlTemplate`",
                    );
                ...
                if (args.crai) indexBlob = new BlobFilehandleWrapper(args.crai);
                    ...
                    indexBlob = new BlobFilehandleWrapper(
                        new XHRBlob(
                            this.resolveUrl(args.urlTemplate + ".crai"),
                        ),
                    );
                else
                    throw new Error(
                        "no index provided, must provide a CRAM index",
                    );
                ...
                sqLines.forEach((sqLine, seqId) => {
                    sqLine.data.forEach((item) => {
                        if (item.tag === "SN") {
                            ...
                            const seqName =
                                this.browser.regularizeReferenceName(
                                    item.value,
                                );
            ...
            hasRefSeq: function (seqName, callback, errorCallback) {
                ...
                this._deferred.stats
                    .then(() =>
                        this.cram.hasDataForReferenceSequence(refSeqNumber),
                    )
                ...
                this.cram
                    ...
                    .then((records) => {
                        if (query.viewAsPairs) {
                            const recs = records.map((f) =>
                                this._cramRecordToFeature(f),
                            );
                            recs.forEach((r) =>
                            ...
                            for (let i = 0; i < records.length; i++) {
                                let feat = this._cramRecordToFeature(
                                    records[i],
                                );
