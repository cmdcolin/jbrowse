    ...
    return declare([SeqFeatureStore], {
        ...
        reload: function (opTree, mask, display) {
            ...
            if (opTree) {
                this.opTree = opTree;
                this.inverse =
                    inverse === undefined ? opTree.get() == "N" : inverse;
                this.stores.mask =
                    opTree.leftChild && !mask ? opTree.leftChild.get() : mask;
                this.stores.display =
                    opTree.rightChild && !display
                        ? opTree.rightChild.get()
                        : display;
                ...
                var haveMaskStore = grabStore(this.stores.mask).then(
                    function (store) {
                ...
                this.gotAllStores.then(function () {
                    thisB.opTree.leftChild = thisB.stores.mask
                        .isCombinationStore
                        ? thisB.stores.mask.opTree
                        : new TreeNode({ Value: thisB.stores.mask });
                    thisB.opTree.rightChild = thisB.stores.display
                        .isCombinationStore
            ...
            this.gotAllStores.then(function () {
                ...
                var grabFeats = function (key) {
                    ...
                    thisB.stores[key].getFeatures(
                        ...
                        function () {
                            d.reject(
                                "failed to load features for " + key + " store",
                            );
                        },
                    );
                    return d.promise;
        ...
        inSpan: function (feature, span) {
            ...
            return feature.get
                ? !(
                      feature.get("start") >= span.end ||
                      feature.get("end") <= span.start
                  )
            ...
            for (var key in features) {
                if (features.hasOwnProperty(key)) {
                    ...
                    for (var span in spans) {
                        if (
                            spans.hasOwnProperty(span) &&
                            this.inSpan(feat, spans[span])
                        ) {
