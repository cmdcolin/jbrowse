         ...
         */
        {
            ...
            constructor: function (args) {
                this.data =
                    args.blob ||
                    new XHRBlob(
                        this.resolveUrl(args.urlTemplate || "data.bigwig"),
                        { expectRanges: true },
                    );
            ...
            _getGlobalStats: function (successCallback, errorCallback) {
                ...
                if (!("scoreMean" in s))
                    s.scoreMean = s.basesCovered
                        ? s.scoreSum / s.basesCovered
                        : 0;
                if (!("scoreStdDev" in s))
                    ...
                    async (bytes) => {
                        try {
                            ...
                            for (var zl = 0; zl < this.numZoomLevels; ++zl) {
                                var zlReduction = data.getUint32(
                                    4 * (zl * 6 + 16),
                                );
                        ...
                        var bptReadNode = function (offset) {
                            ...
                            for (var n = 0; n < cnt; ++n) {
                                if (isLeafNode) {
                                    ...
                                    thisB.refsByName[
                                        thisB.browser.regularizeReferenceName(
                                            key,
                                        )
                                    ] = refRec;
                    ...
                    if (zh && zh.reductionLevel <= 2 * basesPerPx) {
                        var indexLength =
                            i < this.zoomLevels.length - 1
                                ? this.zoomLevels[i + 1].dataOffset -
                                  zh.indexOffset
