...
define(["JBrowse/Util"], function (Util) {
    var Feature = Util.fastDeclare({
        ...
        dataFromVariant: function (variant) {
            ...
            if (variant.FILTER) {
                featureData.filter = {
                    ...
                    values:
                        variant.FILTER === "PASS" ? ["PASS"] : variant.FILTER,
                };
            }
        ...
        _getSOTermAndDescription: function (ref, alt) {
            ...
            alt.forEach((a) => {
                let [soTerm, description] = this._getSOAndDescFromAltDefs(
                    ref,
                    a,
                );
                if (!soTerm) {
                    [soTerm, description] = this._getSOAndDescByExamination(
                        ref,
                        a,
                    );
                }
        ...
        _getSOAndDescFromAltDefs: function (ref, alt) {
            ...
            if (soTerm) {
                let description = this.parser.getMetadata(
                    "ALT",
                    alt,
                    "Description",
                )
                    ? alt +
                      " - " +
                      this.parser.getMetadata("ALT", alt, "Description")
