...
define([
    ...
    return declare(null, {
        renderDetailField: function (
            ...
            if (
                (fieldSpecificFormatter =
                    this.config["fmtDetailField_" + title]) &&
                f
            ) {
                ...
                (fieldSpecificFormatter =
                    this.config["fmtMetaField_" + title]) &&
                !f
            ) {
            ...
            if (
                ...
                (fieldSpecificFormatter =
                    this.config["fmtMetaDescription_" + title]) &&
                !f
            ) {
                fieldMeta = fieldSpecificFormatter(fieldMeta);
            }
        ...
        renderDetailValue: function (
            ...
            if (
                (fieldSpecificFormatter =
                    this.config["fmtDetailValue_" + title]) &&
                f
            ) {
                unsafe = true;
                ...
                (fieldSpecificFormatter =
                    this.config["fmtMetaValue_" + title]) &&
                !f
            ) {
                unsafe = true;
            ...
            domConstruct.create(
                "div",
                {
                    className:
                        "value " +
                        (val.length > 70 && val.indexOf(" ") == -1
                            ? "long "
                            : "") +
                        class_,
                    innerHTML:
                        unsafe || this.config.unsafePopup
                            ? val
                            : Util.escapeHTML(val),
                },
                parent,
        ...
        _valToString: function (val) {
            ...
            if (lang.isArray(val)) {
                return array
                    .map(val, lang.hitch(this, "_valToString"))
                    .join(" ");
