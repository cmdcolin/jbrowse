         ...
         */
        {
                    ...
                    args.selectableFacetFilter ||
                        ...
                        function (facetName, store) {
                            return (
                                ...
                                store.getFacetStats(facetName).avgBucketSize >
                                    1 &&
            ...
            _suppressAsync: function (suppressFlags, callback) {
                ...
                return callback.call(this).then(
                    (retval) => {
                        suppressFlags.forEach(
                            (f) => (this.suppress[f] = false),
                        );
                        return retval;
                    },
                    (err) => {
            ...
            renderInitial: function () {
                ...
                dojo.query(".faceted_tracksel_on_off").onclick(
                    lang.hitch(this, "toggle"),
                );
                ...
                this.busyIndicator = dojo.create(
                    "div",
                    {
                        innerHTML:
                            '<img src="' +
                            this.browser.resolveUrl("img/spinner.gif") +
                            '">',
            ...
            _busy: function (busy) {
                this.busyCount = Math.max(
                    0,
                    (this.busyCount || 0) + (busy ? 1 : -1),
                );
                if (this.busyCount > 0)
                    ...
                    structure: [
                        dojo.map(
                            ...
                            function (facetName) {
                                ...
                                facetName =
                                    { name: "key" }[facetName.toLowerCase()] ||
                                    facetName;
                                return {
            ...
            renderFacetSelectors: function () {
                ...
                var selectableFacets = dojo.filter(
                    ...
                    function (facetName) {
                        return this._isSelectableFacet(
                            facetName,
                            this.trackDataStore,
                        );
                    },
                ...
                dojo.forEach(
                    ...
                    function (facetName) {
                        ...
                        var facetPane = this._renderFacetSelector(
                            facetName,
                            values,
                        );
                    ...
                    function (val) {
                        ...
                        var node = dojo.create(
                            "tr",
                            {
                                ...
                                innerHTML:
                                    '<td class="count"></td><td class="value">' +
                                    val +
                                    "</td>",
                ...
                dojo.forEach(
                    ...
                    function (facetName) {
                        if (facetName == "My Tracks")
                            ...
                            return;
                        var thisFacetCounts =
                            this.trackDataStore.getFacetCounts(facetName);
                        dojo.forEach(
                            this.facetSelectors[facetName] || [],
                            function (selectorNode) {
                                dojo.query(".count", selectorNode).forEach(
                                    function (countNode) {
                                        var count = thisFacetCounts
                                            ? thisFacetCounts[
                                                  selectorNode.facetValue
                                              ] || 0
                                            : 0;
                                        countNode.innerHTML =
            ...
            _updateFacetControl: function (facetName) {
                ...
                if (
                    ...
                    dojo.addClass(
                        titleContent.parentNode.parentNode,
                        "activeFacet",
                    );
            ...
            _updateQuery: function () {
                ...
                (function () {
                    ...
                    if (is_selected(byname["Currently Active"])) {
                        var activeTrackLabels = dojof.keys(
                            this.tracksActive || {},
                        );
                        ...
                        function () {
                            this._suppress("selectionEvents", function () {
                                ...
                                for (
                                    ...
                                    if (item) {
                                        var label =
                                            this.dataGrid.store.getIdentity(
                                                item,
                                            );
                                        if (this.tracksActive[label])
                                            this.dataGrid.rowSelectCell.toggleRow(
                                                i,
                                                true,
                                            );
                                    }
