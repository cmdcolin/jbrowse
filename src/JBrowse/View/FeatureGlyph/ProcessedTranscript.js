define([
    ...
    return declare(SegmentsGlyph, {
            ...
            return this._mergeConfigs(this.inherited(arguments), {
                style: {
                    utrColor: function (feature, variable, glyph, track) {
                        return glyph
                            ._utrColor(
                                glyph.getStyle(feature.parent(), "color"),
                            )
                            .toString();
                    },
        ...
        _makeUTRs: function (parent, subparts) {
            ...
            for (i = 0; i < subparts.length; i++) {
                ...
                if (/^cds/i.test(type)) {
                    ...
                    haveLeftUTR =
                        subparts[i].get("start") == parent.get("start");
        ...
        _utrColor: function (baseColor) {
            return (
                this._palette ||
                (this._palette = Palette.generate(
                    baseColor,
                    "splitComplementary",
                ))
            ).colors[1];
        ...
        _getFeatureHeight: function (viewInfo, feature) {
            ...
            if (this._isUTR(feature))
                return (
                    (height * this.getStyle(feature, "utrHeightPercent")) / 100
                );
