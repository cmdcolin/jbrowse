    ...
    return declare(BoxGlyph, {
        _defaultConfig: function () {
            return this._mergeConfigs(this.inherited(arguments), {
                ...
                style: {
                    transcriptLabelFont:
                        "normal 10px Univers,Helvetica,Arial,sans-serif",
        ...
        _getFeatureRectangle(viewArgs, feature) {
            ...
            if (subfeatures && subfeatures.length) {
                ...
                var transcriptType = this.getConfForFeature(
                    "transcriptType",
                    feature,
                );
                var noncodingType = this.getConfForFeature(
                    "noncodingType",
                    feature,
                );
        ...
        updateStaticElements: function (context, fRect, viewArgs) {
            ...
            for (var i = 0; i < subRects.length; i++) {
                subRects[i].glyph.updateStaticElements(
                    context,
                    subRects[i],
                    viewArgs,
                );
