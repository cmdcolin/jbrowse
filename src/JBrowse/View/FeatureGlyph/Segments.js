    ...
    return declare(BoxGlyph, {
        ...
        renderConnector: function (context, fRect) {
            ...
            if (connectorColor) {
                ...
                var connectorThickness = this.getStyle(
                    fRect.f,
                    "connectorThickness",
                );
                context.fillRect(
                    fRect.rect.l, // left
                    Math.round(
                        fRect.rect.t + (fRect.rect.h - connectorThickness) / 2,
                    ), // top
                    fRect.rect.w, // width
                    connectorThickness,
                );
        ...
        _filterSubSubpart(feature) {
            if (!this._subSubpartsFilter)
                this._subSubpartsFilter =
                    this._makeSubpartsFilter("subSubParts");
        ...
        _makeSubpartsFilter(confKey = "subParts") {
            ...
            if (Array.isArray(filter)) {
                let typeNames = filter.map((typeName) =>
                    typeName.toLowerCase(),
                );
                return (feature) =>
                    typeNames.includes(feature.get("type").toLowerCase());
