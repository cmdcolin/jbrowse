define([
    ...
    return declare(null, {
        ...
        constructor: function (args) {
            this.newTrackKey = args.trackConfig
                ? args.trackConfig.key
                : args.key;
        ...
        _dialogContent: function (store) {
            ...
            nodesToAdd.push(
                dom.create("h2", { innerHTML: "Combination formula" }),
            );
                ...
                opButton.on("change", function (isSelected) {
                    if (isSelected) {
                        ...
                        for (var i = 0; i < numOpLists; i++) {
                            ...
                            whichOpSpan.style.display = opButtons.length
                                ? ""
                                : "none";
                            leftRightSpan.style.display =
                                leftRightButtons.length ? "" : "none";
                        }
                    }
                });
            }
        ...
        _generateSuffixRadioButtons: function (
            ...
            var thisB = this;
            var allowedOps = this._generateSuffixList(
                prefix,
                stringlist,
                offset,
            );
            for (var i in allowedOps) {
                ...
                opButton.on("change", function (isSelected) {
                    if (isSelected) {
                        ...
                        thisB.previewTree = thisB._createPreviewTree(
                            operation,
                            store,
                        );
                        thisB.formulaPreview.innerHTML =
        ...
        _createPreviewTree: function (opString, store) {
            ...
            return this._applyTreeTransform(
                opString.substring(2),
                superior,
                inferior,
            );
        },
        ...
        _applyTreeTransform: function (opString, superior, inferior) {
            ...
            var opNode = this._transformTree(
                opString.substring(2),
                opTree1,
                opTree2,
            );
            ...
            if (candidate == "00") {
                ...
                for (var i in allowedOps) {
                    ...
                    if (displayType == oldType) {
                        var allowedOps2 =
                            this.trackClasses[displayType].allowedOps;
                        for (var j in allowedOps2) {
                            var allowedMaskOps =
                                this.trackClasses["mask"].allowedOps;
