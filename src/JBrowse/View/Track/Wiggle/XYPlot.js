         ...
         */
        {
                ...
                this._getScalingStats(
                    ...
                    dojo.hitch(this, function (stats) {
                        ...
                        if (
                            ...
                            this.makeYScale({
                                fixBounds:
                                    "fixBounds" in this.config
                                        ? this.config.fixBounds
                                        : true,
                                min: scaling.min,
                                max: scaling.max,
                            });
            ...
            _drawFeatures: function (
                ...
                var toY = dojo.hitch(this, function (val) {
                    return (
                        (canvasHeight * (1 - dataScale.normalize(val))) / ratio
                    );
                });
                ...
                dojo.forEach(
                    ...
                    function (p, i) {
                        ...
                        if (score >= 0) {
                            var bgColor = this.getConfForFeature(
                                "style.bg_color",
                                f,
                            );
                            if (bgColor) {
                        ...
                        if (score <= canvasHeight || score > originY) {
                            ...
                            if (score <= originY) {
                                // bar goes upward
                                context.fillStyle = this.getConfForFeature(
                                    "style.pos_color",
                                    f,
                                );
                                var height = originY - score + 1;
                                if (
                                    this.getConfForFeature("noFill", f) == true
                                ) {
                                    height = 1;
                                }
                                thisB._fillRectMod(
            ...
            _maskBySpans: function (
                ...
                for (var index in spans) {
                    if (spans.hasOwnProperty(index)) {
                        var w = Math.ceil(
                            (spans[index].end - spans[index].start) * scale,
                        );
                        var l = Math.round(
                            (spans[index].start - leftBase) * scale,
                        );
                        context.clearRect(l, 0, w, canvasHeight);
                    }
                }
                context.globalAlpha =
                    this.config.style.masked_transparancy || 0.2;
                ...
                if (this.config.variance_band) {
                    ...
                    this.getGlobalStats(
                        dojo.hitch(this, function (stats) {
                            if (
                                "scoreMean" in stats &&
                                "scoreStdDev" in stats
                            ) {
                                var drawVarianceBand = function (
                                    plusminus,
                                    fill,
                                    label,
                                ) {
                                    ...
                                    var varTop = toY(
                                        stats.scoreMean + plusminus,
                                    );
                                    var varHeight =
                                        toY(stats.scoreMean - plusminus) -
                                        varTop;
                                ...
                                var maxColor = new Color(
                                    this.config.style.variance_band_color,
                                );
