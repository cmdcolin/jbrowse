    ...
    return declare([BlockBased, ExportMixin], {
        ...
        _attachDndEvents: function () {
            ...
            for (var eventName in dragEndingEvents)
                on(thisB.dnd, dragEndingEvents[eventName], function () {
                    if (thisB.currentDndSource) {
                        ...
                        thisB.currentDndSource.copyOnly =
                            thisB.sourceWasCopyOnly;
                    }
            ...
            on(thisB.dnd, "OutEvent", function () {
                ...
                dndManager
                    .manager()
                    .overSource(thisB.genomeView.trackDndWidget);
            });
            ...
            when(
                ...
                dojo.hitch(this, function () {
                    ...
                    this.preferencesDialog.run(
                        dojo.hitch(
                            this,
                            function (opTree, newstore, displayType) {
                                this.opTree = opTree;
        ...
        _visible: function () {
            ...
            for (var i in which) {
                ...
                if (which[i]) {
                    var storeType =
                        i == 0 && this.storeType == "mask" ? "mask" : which[i];
                    allTypes[i].allowedOps =
                        this.trackClasses[storeType].allowedOps;
        ...
        renderResultsTrack: function () {
            ...
            if (this._visible().store) {
                ...
                var makeTrack = function () {
                    ...
                    thisB.resultsTrackMenuOptions =
                        thisB.resultsTrack._trackMenuOptions;
                    ...
                    if (thisB._visible().store == thisB.store) {
                        ...
                        if (
                            typeof thisB.resultsTrack._exportFormats ==
                            "function"
                        ) {
                            thisB.config.noExport = false;
                            thisB.exportFormats =
                                thisB.resultsTrack._exportFormats();
                        } else {
        ...
        _resultsTrackConfig: function (trackClass) {
            var config = {
                ...
                metadata: {
                    description:
                        "This track was created from a combination track.",
                },
                type: trackClass,
                autoscale: "local",
            };
        ...
        showRange: function (
            ...
            if (this.resultsTrack && !this.onlyRefreshOuter) {
                ...
                for (var i in stores) {
                    if (
                        stores[i] &&
                        typeof stores[i].loadRegion == "function"
                    ) {
                        var start = startBase;
        ...
        fillBlock: function (args) {
            ...
            if (!this.resultsTrack) {
                this.fillMessage(
                    blockIndex,
                    block,
                    "Drag tracks here to combine them.",
                );
            } else {
        ...
        setTrackClass: function (tclass, type) {
            var allPaths = this.trackClasses[type].resultsTypes.map(
                function (item) {
                    return item.path;
                },
            );
        ...
        _trackMenuOptions: function () {
            ...
            if (this.storeType == "mask") {
                ...
                var maskOrDisplayItems = Object.keys(maskOrDisplay).map(
                    function (i) {
                        return {
                        ...
                        action: function () {
                            ...
                            if (combTrack.opTree) {
                                formulaDiv.innerHTML =
                                    combTrack._generateTreeFormula(
                                        combTrack.opTree,
                                    );
                            }
            ...
            if (
                this._visible().tree &&
                this._visible().tree.getLeaves().length > 1
            ) {
                var operationItems = this._visible().allowedOps.map(
                    function (op) {
                        return {
                            ...
                            title:
                                "change operation of last track to " +
                                combTrack.inWords[op],
                            action: function () {
                                if (combTrack.opTree) {
