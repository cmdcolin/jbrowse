    ...
    return declare([MismatchesMixin, NamedFeatureFiltersMixin], {
        ...
        defaultFeatureDetail: function (
            ...
            if (f.pairedFeature() && track.config.viewAsPairs) {
                ...
                dojo.place(
                    "<div><h1>Paired read details</h1></div><br />",
                    container,
                );
        ...
        defaultAlignmentDetail(track, f, div) {
            ...
            if (
                ...
                this.config.renderPrettyAlignment
            ) {
            ...
            array.forEach(inSheets, (sheet) => {
                try {
                    ...
                    array.forEach(rules, (rule) => {
                        if (rule.styleSheet)
                            includedSheets.push(
                                ...this._getStyleSheets([rule.styleSheet]),
                            );
                    });
        ...
        colorForBase: function (base) {
            ...
            this._baseStyles =
                ...
                function () {
                    var colors = {};
                    try {
                        var styleSheets = this._getStyleSheets(
                        ...
                        array.forEach(styleSheets, function (sheet) {
                            ...
                            array.forEach(classes, function (c) {
                                var match =
                                    /^\.jbrowse\s+\.base_([^\s_]+)$/.exec(
                                        c.selectorText,
                                    );
                                if (match && match[1]) {
                                    var base = match[1];
                                    match =
                                        /\#[0-9a-f]{3,6}|(?:rgb|hsl)a?\([^\)]*\)/gi.exec(
                                            c.cssText,
                                        );
                                    if (match && match[0]) {
        ...
        _alignmentsFilterTrackMenuOptions: function () {
            ...
            var track = this;
            return when(this._getNamedFeatureFilters()).then(
                function (filters) {
            ...
            for (var i = 0; curr_pos < seq.length; i++) {
                ...
                for (var k = 0; k < mismatchesAtCurrentPosition.length; k++) {
                    ...
                    if (mismatch.type == "softclip") {
                        ...
                        for (
                            var l = 0;
                            l < Math.min(mismatch.length, 10000);
                            l++
                        ) {
                            query_str += ".";
                            align_str += " ";
