...
define([
    ...
    return declare([FeatureDetailMixin, NamedFeatureFiltersMixin], {
        defaultFeatureDetail: function (
            ...
            container =
                ...
                domConstruct.create("div", {
                    className:
                        "detail feature-detail feature-detail-" + track.name,
                    innerHTML: "",
                });
        ...
        _isReservedTag: function (t) {
            return (
                this.inherited(arguments) || { genotypes: 1 }[t.toLowerCase()]
            );
        },
        ...
        _renderGenotypes: function (parentElement, track, f) {
            ...
            var gContainer = domConstruct.create(
                "div",
                {
                    className: "genotypes",
                    innerHTML:
                        '<h2 class="sectiontitle">Genotypes (' +
                        gCount +
                        ")</h2>",
                },
                parentElement,
                    ...
                    if (typeof count == "object") {
                        ...
                        domConstruct.create(
                            "td",
                            {
                                ...
                                innerHTML:
                                    Math.round((thisTotal / total) * 10000) /
                                        100 +
                                    "%",
                            },
                            tr,
        ...
        _makeVCFFilters: function (vcfMetadata, inheritedFilters) {
            ...
            var filters = lang.mixin({}, inheritedFilters, {
                hideFilterPass: {
                    ...
                    func: makeFilterFilter(function (filter) {
                        try {
                            return (
                                filter.values.join("").toUpperCase() != "PASS"
                            );
                        } catch (e) {
            ...
            if (vcfMetadata.FILTER) {
                for (var filterName in vcfMetadata.FILTER) {
                    filters[filterName + "_hide"] = function (
                        filterName,
                        filterSpec,
                    ) {
                        return {
                            desc:
                                'Hide sites passing filter "' +
                                filterName +
                                '"',
                ...
                for (var filterName in vcfMetadata.FILTER) {
                    filters[filterName + "_include"] = function (
                        filterName,
                        filterSpec,
                    ) {
                        return {
                            desc:
                                'Include sites passing filter "' +
                                filterName +
                                '"',
                            title: filterName + ": " + filterSpec.description,
                            func: makeFilterFilter(function (f) {
