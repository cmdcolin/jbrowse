    ...
    return declare(
         ...
         */
        {
            ...
            heightUpdate: function (height, blockIndex) {
                ...
                if (!this.inShowRange) {
                    this.heightUpdateCallback(
                        Math.max(this.labelHeight, this.height),
                    );
            ...
            makeTrackLabel: function () {
                ...
                if ((this.config.style || {}).trackLabelCss) {
                    labelDiv.style.cssText +=
                        ";" + this.config.style.trackLabelCss;
                }
            ...
            showRange: function (
                ...
                if (finishCallback)
                    Promise.all(blockShowingPromises).then(
                        finishCallback,
                        finishCallback,
                    );
            ...
            _adjustBlanks: function () {
                if (this.firstAttached === null || this.lastAttached === null) {
                    ...
                    this.leftBlank.style.width =
                        this.firstAttached * this.widthPct + "%";
                    this.rightBlank.style.left =
                        (this.lastAttached + 1) * this.widthPct + "%";
                    this.rightBlank.style.width =
                        (this.numBlocks - this.lastAttached - 1) *
                            this.widthPct +
                        "%";
                }
            },
            ...
            showFatalError: function (error) {
                ...
                this.heightUpdate(
                    domGeom.position(this.fatalErrorMessageElement).h,
                );
                this.updateStaticElements(this.genomeView.getPosition());
            },
            ...
            _handleError: function (error, viewArgs) {
                ...
                if (
                    isObject &&
                    error instanceof Errors.TimeOut &&
                    errorContext.block
                )
            ...
            fillTooManyFeaturesMessage: function (
                blockIndex,
                block,
                scale,
                error,
                ...
                if (this.blocks && this.blocks.length > 0) {
                    ...
                    for (i = 0; i < numBlocks; i++) {
                        if (this.blocks[i]) {
                            ...
                            this.blocks[i].domNode.style.left =
                                i * widthPct + "%";
            ...
            fillMessage: function (blockIndex, block, message, class_) {
                ...
                this.heightUpdate(
                    domGeom.getMarginBox(
                        msgDiv,
                        domStyle.getComputedStyle(msgDiv),
                    ).h,
                    blockIndex,
                );
            },
                    ...
                    try {
                        if (spec.children) {
                            ...
                            if (
                                ...
                                menuConf.onClick = this._makeClickHandler(
                                    spec,
                                    context,
                                );
                            }
            ...
            _makeClickHandler: function (inputSpec, context) {
                ...
                if (typeof inputSpec == "function") {
                    ...
                    inputSpec.content = dojo.hitch(
                        this,
                        "defaultFeatureDetail",
                    );
                }
                ...
                var handler = function (evt) {
                    ...
                    var spec = track._processMenuSpec(
                        dojo.clone(inputSpec),
                        ctx,
                    );
            ...
            _trackDetailsContent: function (additional) {
                ...
                if (
                    ...
                    for (var k in metadata) {
                        var key =
                            this.browser.config.trackSelector.renameFacets[k] ||
                            k;
            ...
            _trackMenuOptions: function () {
                ...
                return [
                    ...
                    },
                    {
                        ...
                        action: function () {
                            new TrackConfigEditor(that.config).show(
                                function (result) {
            ...
            _openDialog: function (spec, evt, context) {
                ...
                if (type == "xhr" || type == "content") {
                    ...
                    if (type == "content")
                        setContent(
                            dialog,
                            this._evalConf(context, spec.content, null),
                        );
                ...
                // open the link in a dialog with an iframe
                    ...
                    var updateIframeSize = function () {
                        ...
                        var width = cDims.w;
                        var height =
                            cDims.h - domGeom.position(dialog.titleBar).h;
                        iframe.width = width;
            ...
            renderRegionHighlight: function (
                ...
                if (
                    highlight.start > args.rightBase ||
                    highlight.end < args.leftBase
                )
                    return;
