                        ...
                        if (
                            Util.basename(
                                c[index.indexConfKey]
                                    ? c[index.indexConfKey].url ||
                                          c[index.indexConfKey].blob.name
                                    : c[index.indexUrlConfKey],
                                index.indexExtensionMap ||
                    ...
                    if (resource.type == index.indexExtension) {
                        ...
                        for (var n in configs) {
                            ...
                            if (
                                Util.basename(
                                    c[this.fileConfKey]
                                        ? c[this.fileConfKey].url ||
                                              c[this.fileConfKey].blob.name
                                        : c[this.fileUrlConfKey],
                                ) == basename
                            ) {
                                // it's a match, put it in
                                c[index.indexConfKey] =
                                    this._makeBlob(resource);
                                return true;
                            }
                        }
                        ...
                        configs[newName][index.indexConfKey] =
                            this._makeBlob(resource);
                        return true;
                    }
                }
                ...
                for (var indexName in singletonIndexes) {
                    for (var fileName in singletonFiles) {
                        for (const m in this.indexTypes) {
                            const index = this.indexTypes[m];
                            if (
                                singletonIndexes[indexName][
                                    index.indexUrlConfKey
                                ]
                            )
                                singletonFiles[fileName][
                                    index.indexUrlConfKey
                                ] =
                                    singletonIndexes[indexName][
                                        index.indexUrlConfKey
