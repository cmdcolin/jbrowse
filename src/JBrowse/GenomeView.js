...
define([
    ...
    return declare([Component, FeatureFiltererMixin], {
        constructor: function (args) {
            ...
            this.outerTrackContainer.className =
                "trackContainer outerTrackContainer";
            ...
            this.locationThumbMover = new locationThumbMover(
                this.locationThumb,
        ...
        _updateVerticalScrollBar: function (newDims) {
            if (typeof newDims.height == "number") {
                ...
                this.verticalScrollBar.container.style.height =
                    ...
                    "px";
                var markerHeight =
                    (newDims.height / (this.containerHeight || 1)) * 100;
            ...
            if (
                typeof newDims.y == "number" ||
                typeof newDims.height == "number"
            ) {
                this.verticalScrollBar.positionMarker.style.top =
                    (((newDims.y || this.getY() || 0) /
        ...
        doubleClickZoom: function (event) {
            ...
            var zoomLoc =
                (event.pageX - dojo.position(this.elem, true).x) /
                this.getWidth();
            if (event.shiftKey) {
        ...
        setRubberHighlight: function (start, end) {
            ...
            var h =
                ...
                function () {
                    ...
                    text.appendChild(
                        document.createTextNode(this.rubberbanding.message),
                    );
        ...
        hideRubberHighlight: function (start, end) {
            if (this.rubberHighlight) {
                this.rubberHighlight.parentNode.removeChild(
                    this.rubberHighlight,
                );
        ...
        slide: function (distance) {
            ...
            new Slider(
                this,
                this.afterSlide,
                Math.abs(distance) * this.getWidth() * this.slideTimeMultiple +
                    200,
        ...
        drawBasePairLabel: function (args) {
            ...
            if (!this.basePairLabels[name]) {
                var scaleTrackPos = dojo.position(
                    args.scaleDiv || this.scaleTrackDiv,
                );
        ...
        sizeInit: function () {
            ...
            this.zoomLevels.unshift(
                this.getWidth() / (this.ref.end - this.ref.start),
            );
            ...
            if (oldStripeCount && oldStripeCount != this.stripeCount) {
                blockDelta = Math.floor(
                    (oldStripeCount - this.stripeCount) / 2,
                );
            ...
            posSize.appendChild(
                document.createTextNode(Util.addCommas(this.ref.end)),
            );
            ...
            for (var n = 1; n < 30; n++) {
                ...
                this.overviewStripes = Math.ceil(
                    refLength / this.overviewStripeBases,
                );
            ...
            var overviewStripePct =
                100 / (refLength / this.overviewStripeBases);
        ...
        overviewTrackIterate: function (callback) {
            ...
            do {
                ...
                overviewTrack &&
                (overviewTrack = overviewTrack.nextSibling)
            );
        },
        ...
        zoomIn: function (e, zoomLoc, steps) {
            ...
            var fixedBp = this.pxToBp(
                pos.x + this.offset + zoomLoc * this.getWidth(),
            );
            this.curZoom += steps;
        ...
        zoomOut: function (e, zoomLoc, steps) {
            ...
            var scale = this.zoomLevels[this.curZoom - steps] / this.pxPerBp;
            var edgeDist =
                this.bpToPx(this.ref.end) -
                (this.offset + pos.x + this.getWidth());
        ...
        replaceTracks: function (trackConfigs) {
            ...
            array.forEach(
                ...
                function (conf) {
                    ...
                    this.trackDndWidget.insertNodes(
                        false,
                        [conf],
                        false,
                        anchor,
        ...
        renderTrack: function (/**Object*/ trackConfig) {
            ...
            this.browser.getStore(trackConfig.store, function (s) {
                ...
                var trackType =
                    trackConfig.type ||
                    thisB.browser.getTrackTypes().trackTypeDefaults[
                        store.config.type
                    ];
                dojo.global.require([trackType], function (class_) {
                    if (typeof class_ === "string") {
                        console.error(
                            "Failed to load module: " + trackConfig.type,
                        );
                        return;
                    }
                    trackClass = class_;
        ...
        setTrackFocus: function (track, state) {
            ...
            if (state === 1) {
                if (this.focusTrack !== null) {
                    ...
                    thisB.browser.publish(
                        "/jbrowse/v1/n/tracks/unfocus",
                        this.focusTrack,
                    );
                    this.focusTrack = null;
                }
        ...
        updateTrackList: function () {
            ...
            for (var i = 0; i < tracks.length; i++) {
                ...
                if (tracks[i].name in this.trackIndices) {
                    newHeights[i] =
                        this.trackHeights[this.trackIndices[tracks[i].name]];
                } else {
                    newHeights[i] = 0;
                }
        ...
        layoutTracks: function () {
            ...
            array.forEach(
                ...
                function (track, i) {
                    ...
                    if (track.shown) {
                        nextTop +=
                            this.trackHeights[i] + this.config.trackPadding;
