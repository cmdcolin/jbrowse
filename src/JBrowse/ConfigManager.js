    ...
    return declare(
         ...
         */
        {
            ...
            constructor: function (args) {
                ...
                this.bootConfig = this._regularizeIncludes([
                    this.bootConfig,
                ])[0];
                ...
                var thisB = this;
                this._getConfigAdaptor(this.bootConfig).then(
                    function (adaptor) {
                        thisB.bootConfig = adaptor.regularizeTrackConfigs(
                            thisB.bootConfig,
                        );
                    },
                );
            ...
            _fillTemplates: function (subconfig, config) {
                ...
                if (lang.isArray(subconfig)) {
                    for (var i = 0; i < subconfig.length; i++)
                        subconfig[i] = this._fillTemplates(
                            subconfig[i],
                            config,
                        );
                } else if (type == "object") {
                    for (var name in subconfig) {
                        if (subconfig.hasOwnProperty(name) && !skip[name])
                            subconfig[name] = this._fillTemplates(
                                subconfig[name],
                                config,
                            );
                    }
                ...
                function _loadRecur(config, upstreamConf) {
                    ...
                    var loads = includes.map((include) => {
                        ...
                        return thisB
                            ...
                            .then((includedData) =>
                                _loadRecur(includedData, newUpstreamConf),
                            );
                    });
            ...
            _regularizeIncludes: function (includes) {
                ...
                return array.map(includes, function (include) {
                    ...
                    if (!("format" in include)) {
                        include.format = /\.conf$/.test(include.url)
                            ? "conf"
                            : "JB_json";
                    }
                    if (
            ...
            _validateConfig: function (c) {
                ...
                if (!c.baseUrl) {
                    this._fatalError(
                        "Must provide a `baseUrl` in configuration",
                    );
                }
            ...
            _mergeConfigs: function (a, b) {
                ...
                for (var prop in b) {
                    if (prop == "tracks" && prop in a) {
                        a[prop] = this._mergeTrackConfigs(
                            a[prop] || [],
                            b[prop] || [],
                        );
                    } else if (
